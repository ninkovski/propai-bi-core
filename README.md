...
* **Stack:** Python, Azure Machine Learning, Azure Functions (para orquestación de datos).
...
* **Stack:** Azure OpenAI (GPT-4), Frameworks de RAG, Azure Functions (para interfaz de chat y flujos).
...
graph TD
    ...
    subgraph Data_Sources["Data Sources"]
        DS_Web["Portales Inmobiliarios (Web Scraping)"] -->|Raw Data| ADF["Azure Data Factory / Azure Functions"]
    ...
    subgraph App_Clients["App / Clients"]
        AOAI <-->|Chat Interface| Functions_Flows["Azure Functions (Workflow Orchestration)"]
        Functions_Flows -->|Integración| Client_App["Web / Dashboard"]
        API_GW --> Client_App
    end
```
...
* **Azure Functions:** Orquesta flujos serverless para la extracción de datos de portales inmobiliarios mediante web scraping, la integración con APIs externas (INEI, Cofopri, Sunarp) y la ejecución de disparadores por tiempo o eventos. Su arquitectura basada en consumo permite escalar por demanda y reducir costo operativo en el MVP.
...
* **Azure Functions (Chat Interface):** Expone el agente como endpoint HTTP/webhook accesible desde el dashboard, con lógica de orquestación y seguridad integrada para fases tempranas del MVP.
* **Flujo:** `Consulta del Usuario` → `Azure Functions` → `LangChain Agent` → `Azure AI Search (Retrieval)` → `Azure OpenAI (Generation)` → `Respuesta Fundamentada`.
...
* [x] Prototipado de flujos de orquestación en Azure Functions.
